...
//!
//! If `tests/nod-tests/fixtures/dylan-lex-shim.lib.obj` exists (built
//! via `nod-driver build --library --project
//! tests/nod-tests/fixtures/dylan-lex-shim.prj -o <that path>`), link
...
//! `tests/nod-tests/fixtures/`), the build still succeeds — the
...
//!     tests/nod-tests/fixtures/dylan-lex-shim.prj \
    ...
    if !shim_obj.is_file() {
        println!(
            "cargo:warning=nod-driver: {} not found — `--lex-with-dylan` will fall back to the Rust lexer. \
             Run `nod-driver build --library --project tests/nod-tests/fixtures/dylan-lex-shim.prj \
