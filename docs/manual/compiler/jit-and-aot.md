...
process lifetime — matching the `keep_forever` rationale from NCL — because
JIT'd machine code must remain executable and SEH-registered as long as any
pointer into it could be on a call stack.
...
- **StaticLibrary shape** (`--library` flag, `src/nod-llvm/src/aot.rs:362`):
  ...
  `nod-driver` for `--lex-with-dylan`) without colliding on `main`. The
  resolver `nod_aot_resolve_relocs` is emitted with external linkage so the
  host can call it once at startup.
...
5. Find `link.exe` via `cc::windows_registry::find` and invoke it with:
   ...
   - `/MAP` to emit a symbol map for crash symbolicaton
...
skips the entire parse → lower → codegen → MCJIT chain on a hit. Delivers
the headline IDE-shell re-eval speedup (documented as ≥10× in the Sprint 37
comment, `src/nod-llvm/src/cache.rs:44`).
...
**Cross-process replay.** Pre-Sprint-38, runtime addresses (class metadata
pointers, literal pool singletons, stub-table entry pointers, inline-cache
slot addresses, generic-function addresses) were baked as `i64` constants
into LLVM IR — usable in-process but stale across processes. Sprint 38
replaced every such bake-site with a named external global (`nod_imm_true__<key>`,
...
- **Engines are leaked.** JIT engines are never freed — machine code pointed
  ...
  `RtlDeleteFunctionTable` + `VirtualFree`) is noted as future work in
  `jit_mm.rs`.
...
---
[Manual home](../index.md) · [Compiler overview](overview.md) · [Codegen](codegen.md)
