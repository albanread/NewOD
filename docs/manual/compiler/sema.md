...
`nod-sema` is the largest front-end crate. It receives a macro-expanded AST and
a namespace context, resolves names, registers classes and slots under multiple
...
It is the standard Python MRO formulation: repeatedly pick the first list-head
that appears in no other list's tail, remove it from all lists, repeat. A
diamond `<e>(<b>, <c>)` where both share ancestor `<a>` correctly produces
...
opt-in; when in doubt the code falls through safely.
...
---
[Manual home](../index.md) · [Compiler overview](overview.md) · [DFM](dfm.md)
